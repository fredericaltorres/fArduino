...
boolean Adafruit_MCP9808::begin(uint8_t addr) {
  ...
  uint16_t v1 = read16(MCP9808_REG_MANUF_ID);
  Serial.print("MCP9808_REG_MANUF_ID=");
  Serial.println(v1);
  if (v1 != 0x0054) return false;

  uint16_t v2 = read16(MCP9808_REG_DEVICE_ID);
  Serial.print("MCP9808_REG_DEVICE_ID=");
  Serial.println(v1);
  if (v2 != 0x0400) return false;
