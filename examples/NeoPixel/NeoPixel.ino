...
void fire(uint8_t wait, uint8_t fireStart, uint8_t fireEnd) {
    ...
    while (j < fireEnd) {
        ...
        Board.Trace(StringFormat.Format("strip.numPixels(): %d", strip.numPixels()));

        for (i = 0; i<strip.numPixels(); i++) {

            uint32_t wheelValue = Wheel((i + j) & 255);
            Board.Trace(StringFormat.Format("%d, // j:%d, i:%d", ((int)wheelValue), j, i));
            strip.setPixelColor(i, wheelValue);
        }
        strip.show();
        int w = flameWaitPattern[flameWaitPatternIndex] * 7 / 2;
        delay(w);
